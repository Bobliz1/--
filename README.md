...
策略基本上更新完毕，核心内容如下：
 ...
 - remark：小市值从价值选股角度，布林带和成交量从动量角度（信号），最大回撤从风险控制角度
   - 此时布林带收紧有两种，一种是绝对收紧，一种是相对收紧。
 - question:布林带的使用貌似有问题，他只能表示波动压缩，不一定是涨还是跌（已解决）
...
- 盈利角度：
  - 价值选股角度：小市值 {5%}
  - 动量因素：成交量以及布林带收紧 {成交量萎缩<80%过去5天平均, 布林带相对收紧<70%历史平均}
- 风控角度：最大回撤 {10%}
- 盈利归因分析：得益于小市值策略的波动性。
- 缺点：
  - 没有稳定的收益动量，还是有些看运气。举个例子，在熊市中，几乎所有持有的小市值股票都会下跌，导致策略失效。（尝试提出的解决方法，加入更多除了市值之外的因子选股）
  - 当前技术有限，只能看收盘价和开盘价做判断，以及只能在两个时间点做处理。（解决方法：尝试加入日内的分析，比如分钟级的策略）
**下一个策略我决定训练**
- 要做到分钟级的策略
- 尝试找到更稳定的动量，以降低蹉跌期，提高夏普比率
- 最近有在读书，往往一个优秀的策略有以下特点：
  - 简单而美，不复杂
  - 换手率高，持有期短
  - 从结果上看，最大挫跌小，挫跌期短，夏普比率高
- 了解更多不同的市场（我目前直到有股票和期货）
